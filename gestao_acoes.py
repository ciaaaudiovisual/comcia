...
def show_gestao_acoes():
                    ...
                    if tipo_selecionado_str.startswith("---"): st.warning("Por favor, selecione um tipo de ação válido.")
                        ...
                        try:
                            ...
                            nova_acao = {
                                ...
                                'tipo_acao_id': str(tipo_info['id']), 'tipo': tipo_info['nome'], 'descricao': descricao, 
                                'data': data.strftime('%Y-%m-%d'), 'usuario': st.session_state.username, 
                                'status': 'Lançado' if lancar_direto else 'A Lançar'
                            }
    ...
    with st.form(key="filter_form"):
        ...
        filtro_status_lancamento = c2.selectbox("Filtrar Status", ["Todos", "A Lançar", "Lançado"], index=1)
    ...
    if not df_display.empty:
        ...
        if filtro_status_lancamento != "Todos": df_display = df_display[df_display['status'] == filtro_status_lancamento]
    ...
    acoes_pendentes_visiveis = df_display[df_display['status'] == 'A Lançar'] if 'status' in df_display.columns else pd.DataFrame()
    ...
    if df_display.empty:
        ...
        for _, acao in df_display.iterrows():
            with st.container(border=True):
                is_launched = acao.get('status') == 'Lançado'
