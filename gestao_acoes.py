...
def show_gestao_acoes():
    ...
    with st.expander("➕ Registrar Nova Ação", expanded=True):
        ...
        pass # Mantido para encurtar a resposta, seu código original deve estar aqui
    ...
    # ======================================================================
    st.warning("NOVAS INFORMAÇÕES DE DIAGNÓSTICO (pode remover depois)")
    st.write(f"Total de ações encontradas após a junção com alunos: {len(df_display)}")

    if not df_display.empty:
        st.write("Contagem de cada status encontrado na lista:")
        st.dataframe(df_display['status'].value_counts())
        
        st.write("Amostra dos 5 primeiros registros para análise completa:")
        st.dataframe(df_display.head())
    
    st.warning("FIM DAS NOVAS INFORMAÇÕES DE DIAGNÓSTICO")
    # ======================================================================
